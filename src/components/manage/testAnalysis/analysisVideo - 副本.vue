     ...
     <Form :label-width="110">
        ...
        <Form-item label="选择知识点：" class="cfix">
          <div v-for="(knowledgeInfo, index) in knowledgeInfos" class="fLeft">
            ...
            <Select v-model="knowledgeInfo.selValue" :key="knowledgeInfo.id" style="width:130px; margin-right: 10px;" @on-change="getKnowledgeLevel(knowledgeInfo)">
        ...
        <Form-item label="附图上传" class="fuTuFile container">
            <form id="testform2" method="post" enctype="multipart/form-data" class="cfix">
            <input name="key" id="key" type="hidden" ref="keyFile2">
            <input name="token" type="hidden" :value="upload_token">
            <input id="userfile2" name="file" type="file" ref="inputFile2" @change="fileOnChange2" />
            <input name="accept" type="hidden" />
          </form>
          <label for="userfile2" class="labelFile fLeft">
            <span></span>
            <em>附图上传</em>
          </label>
          <div class="fLeft fileInfo">
...
export default {
  ...
  methods:{
    ...
    fileOnChange1: function(){
      ...
      console.log(that.uploadFiles[1].selectedFile)
    ...
    fileOnChange2: function(){
      console.log(2)
      var that = this;
      that.uploadFiles[2].selectedFile = that.$refs.inputFile2.value;
      that.uploadFiles[2].urlFile = '';
      console.log(that.uploadFiles[2].selectedFile)
      if (that.uploadFiles[2].selectedFile) {
        var ramdomName = Math.random().toString(36).substr(2) + that.uploadFiles[2].selectedFile.match(/\.?[^.\/]+$/);
        that.$refs.keyFile2.value = ramdomName;
        that.uploadFiles[2].oSelectedFile = '文件：' + that.uploadFiles[2].selectedFile;
      } else {
        return false;
      }
      ...
      $.ajax({
        ...
        xhr: function(){
          ...
          if(myXhr.upload){
            myXhr.upload.addEventListener('progress',function(e) {
              ...
              if (e.lengthComputable) {
                var percent = e.loaded/e.total*100;
                that.uploadFiles[2].progressFile = '上传：' + e.loaded + "/" + e.total+" bytes. " + percent.toFixed(2) + "%";
              }
            }, false);
          }
          return myXhr;
        ...
        success: function(res) {
          ...
          console.log(str)
          that.uploadFiles[2].urlFile = str;
        },
        error: function(res) {  
          console.log("失败:" +  JSON.stringify(res));
          that.uploadFiles[2].urlFile ='上传失败：' + res.responseText;
        }
      });
      return false;
    }
