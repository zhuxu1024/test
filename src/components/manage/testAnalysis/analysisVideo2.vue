...
<div class="mInfo manageCon fRight">
     ...
     <Form :label-width="120">
        ...
        <Form-item label="上传题目：" class="fuTuFile container">
          ...
          <form :id="'testform'+uploadFiles[0].id" method="post" enctype="multipart/form-data" class="cfix">
            ...
            <input :id="'userfile'+uploadFiles[0].id" name="file" type="file" @change="fileOnChange1(uploadFiles[0],0,analysisVideoInfos.questionPattern)" />
        ...
        <Form-item label="上传答案：" class="fuTuFile container" v-if="isShow">
          ...
          <form :id="'testform'+uploadFiles[1].id" method="post" enctype="multipart/form-data" class="cfix">
            ...
            <input name="token" type="hidden" :value="upload_token">
            <input :id="'userfile'+uploadFiles[1].id" name="file" type="file" @change="fileOnChange1(uploadFiles[1],1,analysisVideoInfos.answerType)" />
            <input name="accept" type="hidden" />
          </form>
        ...
        <Form-item label="上传答案：" class="fuTuFile container" v-if="!isShow">
          ...
          <div v-for="(inputList,index) in inputLists" class="cfix">
            <form :id="'testform'+inputList.id" method="post" enctype="multipart/form-data" class="cfix">
              ...
              <input name="token" type="hidden" :value="upload_token">
              <input :id="'userfile'+inputList.id" name="file" type="file" @change="fileOnChange0(inputList,index,formItem.answerFormat2)" /><!-- 'inputFile'+inputList.id -->
              <input name="accept" type="hidden" />
            </form>
        ...
        <Form-item label="试题视频：" class="fuTuFile container">
          ...
          <form :id="'testform'+uploadFiles[3].id" method="post" enctype="multipart/form-data" class="cfix">
            ...
            <input name="token" type="hidden" :value="upload_token">
            <input :id="'userfile'+uploadFiles[3].id" name="file" type="file" @change="fileOnChange1(uploadFiles[3],3,analysisVideoInfos.videoPattern)" />
            <input name="accept" type="hidden" />
          </form>
    ...
    <Modal :title="'第'+num+'步知识点分数选择填充'" v-model="formItem.modal" :mask-closable="false" width="850" @on-ok="onOk">
      <Form :label-width="120">
        ...
        <Form-item label="选择知识点等级：">
            <Radio-group v-model="formItem.radio4" @on-change="getKonwledgeLevelDialog">
                <Radio v-for="levelItem in knowledgeLevelDialog" :label="levelItem.name" :key="levelItem.id">{{levelItem.name}}</Radio>
            </Radio-group>
        </Form-item>
        <Form-item label="选择知识点内容：" class="w200">
          <Select v-model="knowledgesContentDialogId">
        ...
        <Form-item label="分数：" class="w130">
            <Input v-model="standardScoreDialog" placeholder="请输入"></Input>
        </Form-item>
      </Form>
    </Modal>
  </div>
...
export default {
  ...
  methods:{
    ...
    stepShow: function(data){
      if(data == 1){
        for( var i =0; i< this.knowledgeInfoDialogs.length; i++){
          this.knowledgeInfoDialogs[i].selId = '';
        }
        this.knowledgesDialogId = this.formItem.radio4 = '';
        this.standardScoreDialog = 0;
    ...
    onOk: function(){//确定按钮添加分步知识点
      var knowledgesItem = new Object();
          ...
          knowledgesItem.substepNo = this.num;
